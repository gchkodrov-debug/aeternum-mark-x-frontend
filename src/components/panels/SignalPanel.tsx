...
export default function SignalPanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/aeternum/agents/oracle/analyze`, {
        method: "POST",
  ...
  return (
    <div className="hud-panel">
      <div className="hud-panel-header">
        ...
        <span className="hud-panel-title">SIGNAL PIPELINE</span>
                      ...
                      {typeof item === "object" && item !== null
                        ? Object.entries(item as Record<string, unknown>).map(([k, v]) => (
                            <span key={k} style={{ marginRight: "8px" }}>
                              <span className="hud-metric-label">{formatLabel(k)}:</span>{" "}
                              <span className={`hud-metric-value ${valueClass(v)}`}>{formatValue(v)}</span>
                            </span>
                          ))
                        : formatValue(item)}
                    </div>
