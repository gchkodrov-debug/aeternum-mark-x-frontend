...
export default function RiskDashPanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/aeternum/agents/risk/analyze`, {
        method: "POST",
  ...
  const riskClass = riskLevel
    ? riskLevel.toLowerCase() === "low" || riskLevel.toLowerCase() === "minimal"
      ? "hud-tag--bullish"
      : riskLevel.toLowerCase() === "high" || riskLevel.toLowerCase() === "critical" || riskLevel.toLowerCase() === "extreme"
      ? "hud-tag--bearish"
      : "hud-tag--neutral"
    : "hud-tag--neutral";
  ...
  return (
    <div className="hud-panel">
      <div className="hud-panel-header">
        ...
        <span className="hud-panel-title">RISK DASHBOARD</span>
      ...
      <div className="hud-panel-body">
        {data ? (
          <>
            {riskLevel && (
              <span className={`hud-tag ${riskClass}`}>
                ...
                .map(([key, value]) => (
                  <div key={key}>
                    ...
                    {(value as unknown[]).map((item, i) => (
                      <div key={i} className="hud-list-item" style={{ paddingLeft: "12px" }}>
