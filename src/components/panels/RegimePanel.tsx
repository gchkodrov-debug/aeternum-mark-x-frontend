...
export default function RegimePanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/aeternum/agents/regime/analyze`, {
        method: "POST",
  ...
  const regimeType = data && typeof data.regime_type === "string" ? data.regime_type : null;
  const regimeClass = regimeType
    ? regimeType.toLowerCase().includes("bull")
      ? "hud-tag--bullish"
      : regimeType.toLowerCase().includes("bear")
      ? "hud-tag--bearish"
      : "hud-tag--neutral"
    : "hud-tag--neutral";
  ...
  return (
    <div className="hud-panel">
      <div className="hud-panel-header">
        ...
        <span className="hud-panel-title">MARKET REGIME</span>
      ...
      <div className="hud-panel-body">
        {data ? (
          <>
            {regimeType && (
              <span className={`hud-tag ${regimeClass}`}>
