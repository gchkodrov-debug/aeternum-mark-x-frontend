...
export default function CapitalPanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/aeternum/agents/capital/analyze`, {
        method: "POST",
  ...
  const phase = data && typeof data.phase === "string" ? data.phase : null;
  const progress = data && typeof data.progress === "number" ? data.progress : null;

  return (
    <div className="hud-panel">
      <div className="hud-panel-header">
        <span className="hud-panel-dot" data-status={status} />
        <span className="hud-panel-title">CAPITAL &amp; PHASE</span>
      ...
      <div className="hud-panel-body">
        {data ? (
          <>
            {phase && (
              <span className="hud-tag hud-tag--neutral">
            ...
            {progress !== null && (
              <>
                <div className="hud-metric">
                  <span className="hud-metric-label">Progress</span>
                  <span className="hud-metric-value">{(progress * 100).toFixed(1)}%</span>
                </div>
                <div className="hud-bar">
                  <div className="hud-bar-fill" style={{ width: `${progress * 100}%` }} />
                </div>
              </>
            )}
            <div className="hud-list">
              {Object.entries(data)
                .filter(([k, v]) => k !== "phase" && k !== "progress" && (typeof v !== "object" || v === null))
                .map(([key, value]) => (
